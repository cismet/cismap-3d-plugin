...
public class StaticRegionTerrain {
    ...
    public static final String sceneGraphID = "terrain_region";
     ...
     */
    public StaticRegionTerrain(final float yOffset, final AssetManager am, final Camera cam) {
        regionTerrainNode = (TerrainQuad)am.loadModel(sceneGraphID + ".j3o");
        final TerrainLodControl controlSurround = new TerrainLodControl(regionTerrainNode, cam);
     ...
     */
    public StaticRegionTerrain(final float yOffset,
        ...
        final File worldFile = new File(srcFolder, dgmResolution + "terrain_region.wld");
        final File heightDataFile = new File(srcFolder, dgmResolution + "terrain_region.txt");
        final Coord cDeltasSurround = inst.getDeltasFromWorldFile(worldFile);
...
//              Coord urSurround = cllSurround.plusNew(dgmResolution*cDeltasSurround.x(), dgmResolution*cDeltasSurround.y());
        ...
        regionTerrainNode.setLocalTranslation((float)(cDeltasSurround.x() * dgmResolution) / 2 / 3,
            -ts.MIN_TERRAIN_HEIGHT,
            (float)(cDeltasSurround.y() * dgmResolution)
                    / 2
                    / 3);
        ...
        // set Box for region
        final BBox boxSurround = ts.getBBoxFullTerrain().clone();
