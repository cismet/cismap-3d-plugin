...
public class Wuppertal3D {
    ...
    private static final String wmsExampleQueryRegion =
        "http://www.wms.nrw.de/geobasis/wms_nw_dop40?REQUEST=GetCapabilities&SERVICE=wms&VERSION=1.3.0?&VERSION=1.1.1&REQUEST=GetMap&BBOX=374327.,5681000,375327.,5682000&WIDTH=512&HEIGHT=512&SRS=EPSG:25832&FORMAT=image/png&LAYERS=WMS_NW_DOP40";
    private static final int iTextureResolutionRegion = 2048;
    private static final int iDGMResolutionRegion = 128;
    ...
    private static final String wmsExampleQueryStatic =
        "http://s102w484:8399/arcgis/services/WuNDa-Orthophoto-WUP/MapServer/WMSServer?&VERSION=1.1.1&REQUEST=GetMap&BBOX=374327.,5681000,375327.,5682000&WIDTH=4096&HEIGHT=4096&SRS=EPSG:25832&FORMAT=image/png&LAYERS=7&STYLES=default";
    private static final int iTextureResolutionStatic = 4096;
    private static final int iDGMResolutionStatic = 512;
    ...
    private static final String wmsExampleQueryPatched =
        "http://s102w484:8399/arcgis/services/WuNDa-Orthophoto-WUP/MapServer/WMSServer?&VERSION=1.1.1&REQUEST=GetMap&BBOX=374327.,5681000,375327.,5682000&WIDTH=4096&HEIGHT=4096&SRS=EPSG:25832&FORMAT=image/png&LAYERS=7&STYLES=default";
    private static final int iPatchSizePatched = 200;
    private static final File srcFolderGeometryPatched = new File("C:\\Temp\\ddddaten\\rohdaten");
    ...
    /**
     * Creates a new Wuppertal3D object.
     *
     * @param  assetManager       DOCUMENT ME!
     * @param  cam                DOCUMENT ME!
     * @param  rootNode           DOCUMENT ME!
     * @param  statusTextStatic   DOCUMENT ME!
     * @param  statusTextDynamic  DOCUMENT ME!
     * @param  bCreationMode      DOCUMENT ME!
     */
    public Wuppertal3D(final DesktopAssetManager assetManager,
            final Camera cam,
            final Node rootNode,
            final BitmapText statusTextStatic,
            final BitmapText statusTextDynamic,
        ...
        if (bCreationMode) {
            regionTerrain = new StaticRegionTerrain(
                    yoffsetRegion,
                    iDGMResolutionRegion,
                    srcFolderGeometryRegion,
                    dstFolderRegion,
                    iTextureResolutionRegion,
                    wmsExampleQueryRegion,
                    am,
                    cam,
                    this);
            staticTerrain = new StaticTerrain(
                    yoffsetStatic,
                    iDGMResolutionStatic,
                    srcFolderGeometryStatic,
                    dstFolderStatic,
                    iTextureResolutionStatic,
                    wmsExampleQueryStatic,
                    am,
                    cam,
                    this);
            patchedTerrain = new DynamicPatchedTerrain(
                    yoffsetPatched,
                    offsetPatchedterrain,
                    srcFolderGeometryPatched,
                    dstFolderPatched,
                    iPatchSizePatched,
                    wmsExampleQueryPatched,
                    iTextureResolutionLow,
                    iTextureResolutionHigh,
                    assetManager,
                    cam,
                    this);
            patchedBuildings = new DynamicPatchedBuildings(
                    srcFolderGeometryBuildings,
                    dstFolderGeometryBuildings,
                    offsetPatchedBuildings,
                    iPatchSizeBuildings,
                    this,
                    false,
                    true);
        ...
        threadPatchController = new Thread() {
                ...
                public void run() {
                    try {
                        ...
                        final int iPositionsCache = 3;
                        final Vector3f[] lastPositions = new Vector3f[iPositionsCache];
                        ...
                        while (bThreadControllerRunning) {
                            ...
                            final double distSum = getSumDistance(lastPositions);
                            // Case: Standing
                            ...
                            System.gc();
                            am.clearCache();
                            System.out.print(camStatus + " GeoJobs = " + geometryLoadJobs.size() + " TexJobs = "
                                        + textureLoadJobs.size() + "  ");
                        }
                    } catch (Exception e) {
                ...
                public double getSumDistance(final Vector3f[] positions) {
                    double dist = 0;
            ...
            threadsGeometryLoader[iThread] = new Thread() {
                    ...
                    public void run() {
                        try {
                            ...
                            final Vector<GeometryLoadJob> jobs = new Vector<GeometryLoadJob>();
                            while (bGeometryLoadingThreadsRunning) {
                                ...
                                if (jobs.size() == 0) {
                                    ...
                                    for (final GeometryLoadJob job : jobs) {
                                        job.doJob(am);
                                    }
                                    jobs.clear();
                                }
            ...
            threadsTextureLoader[iThread] = new Thread() {
                    ...
                    public void run() {
                        try {
                            ...
                            final Vector<TextureLoadJob> jobs = new Vector<TextureLoadJob>();
                            while (bTextureLoadingThreadsRunning) {
                                ...
                                if (jobs.size() == 0) {
                                    ...
                                    for (final TextureLoadJob job : jobs) {
                                        job.doJob(am);
                                    }
                                    jobs.clear();
                                }
     ...
     */
    public void addGeometry2BeLoaded(final Integer id,
            final GeometryLoadListener listener,
            final LOADPRIORITY prio,
            final GEOMETRYJOBTYPE type) {
        synchronized (geometryLoadJobs) {
            final GeometryLoadJob job = new GeometryLoadJob(id, prio, listener, type);
            // if(!geometryLoadJobs.contains(job)){
            final int size = geometryLoadJobs.size();
            switch (prio) {
                case LOW: {
     ...
     * @param  prio        DOCUMENT ME!
     */
    public void addTexture2BeLoaded(final Integer id,
            final int resolution,
            final TextureLoadListener listener,
            final LOADPRIORITY prio) {
        synchronized (textureLoadJobs) {
            final TextureLoadJob job = new TextureLoadJob(id, resolution, prio, listener);
            // if(!textureLoadJobs.contains(job)){
            final int size = textureLoadJobs.size();
            switch (prio) {
                case LOW: {
    ...
    /**
     * private BBox position2RoundedBBox( Vector3d position){ double factorx = Math.floor(position.x/200); double x0 =
     * factorx*200; double x1 = (factorx+1)*200; double factory = Math.floor(position.z/200); double y0 = factory*200;
     * double y1 = (factory+1)*200; return new BBox(x0, y0, x1, y1); } private String getIdByCamPosition(Vector3f
     * currentCamPosition){ float xGlobal = currentCamPosition.x+vecGlobalOffset.x; float zGlobal =
     * vecGlobalOffset.z-currentCamPosition.z; xGlobal-=300000;// int kmx = (int)FastMath.floor(xGlobal/1000);
     * zGlobal-=5600000; int kmz = (int)FastMath.floor(zGlobal/1000); float restx = xGlobal%(kmx*1000); float restz =
     * zGlobal%(kmz*1000); int irx = (int)FastMath.floor(restx/200)+1; int irz = (int)FastMath.floor(restz/200); int
     * iPart = irx+irz*5; if(iPart<10){ return kmx+""+kmz+"0"+iPart; } else{ return kmx+""+kmz+""+iPart; } } public
     * Vector3d getGlobalPositionByCam(Vector3f currentCamPosition,boolean bRemoveUTMZone){ float xGlobal =
     * currentCamPosition.x+vecGlobalOffset.x; if(bRemoveUTMZone&& xGlobal>999999){ xGlobal-=32000000; } float zGlobal =
     * vecGlobalOffset.z-currentCamPosition.z; Vector3d vecGlobal = new Vector3d(xGlobal, currentCamPosition.y,
     * zGlobal); return vecGlobal; } private boolean shouldCheckCamPosition(){
     * if(currentCamPosition.distance(cam.getLocation())>cameraMoveDistThres){ return true; } return false; }.
     *
    ...
    public void update(final float tpf) {
        ...
        if (timeSinceLastTextUpdate > 0.3) {
            final int iSizeTexTiles = textureLoadJobs.size();
            final int iSizeGeoTiles = geometryLoadJobs.size();
            ...
            statusTextDynamic.setText("                                                            "
                        + String.valueOf(iSizeGeoTiles) + "\n"
                        + "                                                            "
     ...
     * Texture t = new Texture2D(new AWTLoader().load(buffImagePart, true)); return t; }.
     *
     * @param  name  DOCUMENT ME!
     */
    public void switchVisibility(final String name) {
        final Node n = (Node)rootNode.getChild(name);
        if (n != null) {
            final Vector3f vec = n.getLocalTranslation();
            if (vec.y >= -50000) {
