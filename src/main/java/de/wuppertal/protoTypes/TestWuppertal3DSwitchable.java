...
public class TestWuppertal3DSwitchable extends SimpleApplication implements ActionListener {
    ...
    public static void main(final String[] args) {
        final TestWuppertal3DSwitchable app = new TestWuppertal3DSwitchable();
...
//              settings.setHeight(800);
    ...
    public void simpleInitApp() {
        ...
        final BitmapText statusTextStatic = new BitmapText(guiFont, false);
        ...
        final BitmapText statusTextDynamic = new BitmapText(guiFont, false);
        ...
        terrainSystem = new Wuppertal3D((DesktopAssetManager)assetManager,
                cam,
                rootNode,
                statusTextStatic,
                statusTextDynamic,
                false);
        final BBox box = terrainSystem.getBBoxFullTerrain();
        ...
        final String dateInString = "23-06-2014 07:00:00";
        final Date date;
        try {
...
//              sky.updateTimeIncrement(cam.getLocation());
        final Vector3f origin = cam.getWorldCoordinates(new Vector2f(settings.getWidth() / 2, settings.getHeight() / 2),
                0.0f);
        final Vector3f direction = cam.getWorldCoordinates(new Vector2f(
                    settings.getWidth()
                            / 2,
                    settings.getHeight()
                            / 2),
        ...
        final int numCollisions = rootNode.collideWith(ray, results);
        if (numCollisions > 0) {
