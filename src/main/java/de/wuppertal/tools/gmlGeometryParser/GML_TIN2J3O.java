...
public class GML_TIN2J3O extends AbstractGmlParser {
    ...
    /**
     * Creates a new GML_TIN2J3O object.
     *
     * @param  fileRoot  DOCUMENT ME!
     */
    public GML_TIN2J3O(final File fileRoot) {
        super(fileRoot);
     ...
     * DOCUMENT ME!
     *
     * @param   globalOffset  DOCUMENT ME!
     *
     * @return  DOCUMENT ME!
     */
    public synchronized HashMap<String, BBox> getAllBBoxesReal(final Coord globalOffset) {
        final HashMap<String, BBox> mapRet = new HashMap<String, BBox>();
        for (final Entry<String, BBox> entry : mapBBoxRealByID.entrySet()) {
            final BBox valBBox = entry.getValue();
            final String valId = entry.getKey();
            if ((valBBox.getWidth() != PATCHSIZE) || (valBBox.getHeight() != PATCHSIZE)) {
                final BBox bbox = position2RoundedBBox(valBBox.getCenter2d(), globalOffset);
     ...
     * @return  DOCUMENT ME!
     */
    private BBox position2RoundedBBox(final Coord position, final Coord globalOffset) {
        final double posx = position.x() - globalOffset.x();
        final double posy = position.y() - globalOffset.y();
        final double factorx = Math.floor(posx / PATCHSIZE);
        final double x0 = (factorx * PATCHSIZE) + globalOffset.x();
        final double x1 = ((factorx + 1) * PATCHSIZE) + globalOffset.x();
        final double factory = Math.floor(posy / PATCHSIZE);
        final double y0 = (factory * PATCHSIZE) + globalOffset.y();
        final double y1 = ((factory + 1) * PATCHSIZE) + globalOffset.y();
        ;
     ...
     * DOCUMENT ME!
     *
     * @param  j3oDestFile  DOCUMENT ME!
     */
    public void setDestFile(final File j3oDestFile) {
    ...
    protected void finishedParsing(final File f) {
        ...
        for (final Entry<GMLTYPE, Vector<CoordVector>> entry : map.entrySet()) {
            ...
            final Vector2f[] tx = new Vector2f[vertices.length];
            final int[] indexes = new int[vertices.length]; // { 2,0,1, 1,3,2 };
            final BBox boxTile = getBBox(vecvecTriangles);
            int iVertices = 0;
            for (final CoordVector vec : vecvecTriangles) {
                for (final Coord c : vec.getCoords()) {
                    ...
                    tx[iVertices] = calcTextureCoord(boxTile.getMinX(),
                            boxTile.getMinY(),
                            PATCHSIZE,
                            vertices[iVertices].x,
                            vertices[iVertices].z);
                    iVertices++;
                }
                final Vector3f normal;                      // = getNormal(vertices[iVertices-1] ,
                                                            // vertices[iVertices-2], vertices[iVertices-3]);
                // PSEUDO NORMAL
            ...
            final Geometry geo = new Geometry("geo", mesh); // using our custom mesh object
            geo.setMaterial(mat);
     ...
     * @return  DOCUMENT ME!
     */
    private Vector2f calcTextureCoord(final double minX,
            final double minY,
            final double tilesize,
            final float x,
            final float y) {
        final float txx = (float)((x - minX) / tilesize);
        final float txy = (float)((y - minY) / tilesize);
     ...
     * DOCUMENT ME!
     *
     * @param   vecvecTriangles  DOCUMENT ME!
     *
     * @return  DOCUMENT ME!
     */
    private BBox getBBox(final Vector<CoordVector> vecvecTriangles) {
        BBox box = null;
        for (final CoordVector vec : vecvecTriangles) {
            if (vec.getBBox() == null) {
                vec.updateBBox2d();
...
//                      int i=0;
    ...
    protected void parseGMLClass(final String currentName,
            ...
            Element e,
            final Iterator<Element> processDescendants) {
        final GMLTYPE type = GMLTYPE.parseTye(currentName);
    ...
    private Vector3f[] normalizeNormals(final Vector3f[] coords, final Vector3f[] normals) {
        // HashMap<String, Vector3f> mapCoordsByID = new HashMap<String, Vector3f>();
        final HashMap<String, Vector<Vector3f>> mapNormalsByID = new HashMap<String, Vector<Vector3f>>();
        int i = 0;
        for (final Vector3f vec : coords) {
            final String id = getCoordID(vec);
            if (mapNormalsByID.containsKey(id)) {
                mapNormalsByID.get(id).add(normals[i]);
            } else {
                final Vector<Vector3f> vecAllNormals = new Vector<Vector3f>();
        ...
        for (final Vector3f vec : coords) {
            ...
            ;
            final Vector<Vector3f> allNormalsAtThisPoint = mapNormalsByID.get(id);
            int iNormalsAtThisPoint = 0;
            for (final Vector3f oldNormal : allNormalsAtThisPoint) {
                if (newNormal == null) {
                    newNormal = oldNormal;
                    iNormalsAtThisPoint++;
                } else {
     ...
     * @return  DOCUMENT ME!
     */
    private String getCoordID(final Vector3f vec) {
        final int dx = (int)(Math.round(vec.x * 100));
        final int dy = (int)(Math.round(vec.y * 100));
        final int dz = (int)(Math.round(vec.z * 100));
